  ...
  const CartView = (props) => {
    ...
    // APPLY COUPON - Dynamic from backend (POST /coupons/apply)
    const applyCoupon = async () => {
      ...
      try {
        ...
        // Call the apply coupon endpoint (POST with body)
        const response = await axiosInstance.post("/coupons/apply", {
          couponCode: code
        });
        
        // Backend returns { success: true, data: { couponCode, discount, discountType, timesUsed } }
        ...
        if (isValid && couponData) {
          ...
          toast.success(`Coupon "${couponData.couponCode}" applied! You saved ₹${Math.round(calculatedDiscount)}`);
        } else {
...
<motion.img
  ...
  onError={(e) => {
    ...
    e.target.src = "https://placehold.co/96x128?text=No+Image";
  }}
/>
                      </div>
